...
module.exports = function (grunt) {
    ...
    grunt.registerMultiTask('insert', 'Insert code from one file into another.', function () {
        ...
        this.files.forEach(function (f) {
            ...
            if (Array.isArray(f.dest)) {
                for (var dest_key in f.dest) {
                    var destination = f.dest[dest_key];

                    // Read destination file
                    if (!grunt.file.exists(destination)) {
                        grunt.log.warn('Destination file "' + destination + '" not found.');
                    } else {
                        var dest = grunt.file.read(destination); // Read file source.
                    }

                    var result = dest.replace(f.match, src);

                    // Write the temporary destination file.
                    grunt.file.write(destination + "_tmp", result);

                    // overwrite the original with the temporary file
                    grunt.file['delete'](destination);
                    grunt.file.copy(destination + "_tmp", destination);
                    grunt.file['delete'](destination + "_tmp");

                    // Print a success message.
                    var input = f.src.toString().replace(/.*\//, ""); // Remove path
                    var output = destination.toString().replace(/.*\//, ""); // Remove path
